...
`listSitemapEntries` (MVP-021) is the sitemap query: PUBLISHED is an allow-list (so any future status is excluded by default), categories are included only while they have a PUBLISHED product, both queries are ordered by slug and bounded by `maxEntries` (one extra product is fetched purely to report truncation), and an invalid limit throws `RangeError`. It uses the existing indexes; no migration.
