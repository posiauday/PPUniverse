...
A second CI run followed this push (new code: the two fixes above); its confirmation is appended to section 7 once green, following the same discipline as every other push in this decision's history — this section is not overwritten to make history read as if the first push were already final.
