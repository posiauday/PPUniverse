...
## Moderation/admin
`POST /api/admin/products/{id}/suspend`, `POST /api/admin/entitlements/grant`, `GET /api/admin/audit` — not yet implemented (MVP-019 and later). ~~`GET /api/admin/submissions`, `POST /api/admin/submissions/{id}/decision`~~ **removed 2026-09-24** (`docs/open-questions.md` item 62) — that pair presumed a third-party submission/moderation-decision shape belonging to MVP-013 (Submission review queue), which is superseded under the first-party-only model and has no approved successor. MVP-012's own quality gate is the `POST /api/admin/products/{id}/publish` route listed above, not a separate moderation-decision endpoint.
