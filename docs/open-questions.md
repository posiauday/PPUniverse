...
6. **Part 1 CLOSED (2026-09-21, direct product-owner instruction — see `docs/final-decisions.md`)**: compatibility is modeled as one or more entries per product, each with a platform area (Power Apps, Power Automate, Power BI, Dataverse, Power Pages, Copilot Studio, Microsoft Fabric), a structured minimum supported release wave (release year + wave 1 or 2), and concise compatibility notes. **Part 2 (evidence method) — partially resolved**: the evidence-status vocabulary (Tested / Creator Declared / Not Verified, with definitions, and Tested requiring an Evidence Summary and Last Verified Date) was approved in the same instruction. Still open: who may assign each status and what review or verification process a moderator follows (MVP-012/013 scope).
