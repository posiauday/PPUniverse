...
- The required check is **"Accessibility (axe + Playwright)"** in `.github/workflows/ci.yml`. Since the "Accessibility suite mitigation" decision (2026-09-23), that name belongs to an **aggregator job**, not a single test run: it `needs` a 4-way matrix job (`Accessibility shard 1/4` … `4/4`), each shard on its own independent Postgres service container, and fails if any shard failed (`needs.accessibility.result != 'success'`, checked with `if: always()`). The aggregator keeps the pre-sharding job's exact name so branch protection needed no reconfiguration. Every engine (chromium, firefox, webkit), every width (320/375/768/1280) and every rule is preserved in every shard — nothing was trimmed, skipped or moved to a reduced required set. Sharding uses Playwright's own `--shard=<N>/4` flag against the sorted, fully-resolved test list, which is deterministic (confirmed by repeated `--list` runs producing identical partitions across shards). The unconditional self-check (`negative-controls.spec.ts`, 12 tests) runs a second time in **every** shard, in all three engines, as its own separate `playwright test` invocation — so a gate defect is caught no matter which shard would have exposed it, not only in the shard the file happens to sort into.
- **Required for merge**: once the fixes and the harness had landed together in MVP-023's pull request, it is made a required status check on `develop` by a branch-protection rule (see `docs/final-decisions.md`, stop-gate response item 1; the applied settings are recorded in the pull request and in `docs/open-questions.md` item 17). Sharding did not change this: the required-check name is unchanged (see above).
- **Caution for anyone editing this workflow**: the required-check name (`Accessibility (axe + Playwright)`) now belongs to the small aggregator job (`accessibility-summary` in `ci.yml`, a five-line shell script), not to the job that actually runs the tests (`accessibility`, the 4-shard matrix). Renaming the aggregator job — including a well-intentioned "tidy the job names" pass — silently detaches branch protection from this gate, because GitHub matches required checks by name, not by which job does the real work. If the aggregator's name ever needs to change, update the branch-protection rule in the same pull request, not after.
- **Budget (revised 2026-09-23, "Accessibility suite mitigation")**: the governing measure is now **per-shard test execution time**, target under 5 minutes, ceiling 8 minutes. Job wall-clock is still reported in every run's summary but no longer gates the standing condition — per-shard test execution is the leading indicator; the fixed per-job overhead (~118s: install, browser cache, build) is a lagging one. The standing condition carries over in this new form: stop before a future story adds page states once per-shard execution exceeds target, or before merging any run whose per-shard execution exceeds the ceiling — in both cases, report and decide before continuing, the same as the original trigger this decision resolved. Engines, widths and rules are still never dropped to fit any budget.
...
- **Artifacts**: the HTML report, JUnit and JSON results, `summary.md` and failure traces are uploaded per shard (`accessibility-report-shard-<N>`, 14 days), covering both the shard's main-pool run and its self-check run (namespaced by `E2E_OUTPUT_SUFFIX`/`E2E_SUMMARY_PATH` so the two invocations in one shard's job never overwrite each other's output — confirmed locally: without namespacing, a second `playwright test` invocation in the same job silently overwrote the first's `results/summary.md`).
...
- **Resolved (2026-09-23, "Accessibility suite mitigation"):** the MVP-018 merged run breached the then-10-minute wall-clock ceiling at 10m7s, firing the standing mitigation trigger established at that merge. Sharding was implemented as its own CI-only change (branch `chore/accessibility-suite-sharding`, no product code): 4 shards via Playwright's deterministic `--shard`, verified via `--list` to sum to the exact pre-sharding main-pool count (711) with no drops or duplicates (178+178+178+177). The failure-evidence/BUG-014 instrumentation (`packages/e2e/src/failure-evidence.ts`) was verified, not assumed, to survive sharding, via a deliberate forced failure under `--shard=1/1` that was reverted before commit. Full arithmetic, constraint-by-constraint evidence and the new standing budget are recorded in `docs/final-decisions.md`.
