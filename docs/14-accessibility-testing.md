...
- **Known issue (2026-09-21, BUG-012):** the job serves a *production* build, and `@ppu/db` does not cache its Prisma client in production, so each query opens a new connection pool. Under the suite's concurrency Postgres can run out of connections, which fails a few tests with HTTP 500. This is a product defect, not a test flake; retries and fewer workers would only hide it. The job must not be a required check until BUG-012 is fixed (open question 43). Remove this note when it is.
