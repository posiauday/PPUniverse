...
Modular monolith (ADR 001) expressed as a monorepo with two runtime apps sharing domain packages, mirroring the TRD's domain list: Identity, Catalog, Content, Commerce, Entitlements, ~~Creator~~ (superseded 2026-09-24, `docs/final-decisions.md`, "First-party-only publishing model" — first-party authoring is Catalog/Publishing work, MVP-012, not a separate domain), Moderation (scope under review, same decision), Reviews, Search, Notifications, Analytics, Administration.
...
- **Authorization model**: role-based with per-domain permission checks, deny-by-default (NFR-002).
  - Roles: Guest, Member, ~~Creator~~, Moderator, SupportAgent, Admin. **`Creator` is superseded 2026-09-24** (`docs/final-decisions.md`, "First-party-only publishing model") — no `CREATOR` role is created; the real, built `ADMIN` role (MVP-017, MVP-020) is the sole authoring/publishing authority. Whether `Moderator` survives depends on MVP-013's still-open successor scope.
  - Every mutating use case in `packages/domain/*` declares the roles/ownership conditions it requires (e.g., ~~"product update requires Creator role AND ownership of the product OR Admin role"~~ **"product update requires the `ADMIN` role" — reworded 2026-09-24, same decision**). This check runs in the domain layer, not in UI or route handlers, so it cannot be bypassed by adding a new entry point.
...
1. **Upload**: ~~creator~~ **the publishing administrator (reworded 2026-09-24, first-party-only decision)** uploads through a pre-signed upload URL into a private quarantine bucket/prefix. Client never gets a public URL. Declared file type, size limit, and archive nesting depth are validated both client-side (fast feedback) and server-side (authoritative) before the upload URL is issued.
...
7. **Test coverage required before first-party publishing begins** (reworded 2026-09-24; security gate in `docs/08`): oversized file, type-mismatch file, nested-archive bomb, EICAR-style test file for the scan adapter, expired-signed-URL reuse, and unauthorized-download attempts against another user's entitlement.
...
- **Milestones**: group by the wave sequence in Section 8 — `M1 Foundation`, `M2 Trust Core Loop` (catalog → checkout → entitlement → download), `M3 ~~Creator & Moderation~~ First-Party Publishing Loop` (reworded 2026-09-24, `docs/final-decisions.md`, "First-party-only publishing model"), `M4 Account, Reviews & Content` (P1), `M5 Launch Gate`.
...
| 3 | MVP-004, MVP-005, ~~MVP-011~~, MVP-010, MVP-017 (P1), MVP-018 (P1), MVP-020, MVP-023 | Search/filter and product-detail build on catalog; free-entitlement flow only needs accounts + file pipeline; consent/deletion only needs accounts; accessibility gate begins once catalog pages exist. **MVP-011 is Superseded 2026-09-24** (`docs/final-decisions.md`, "First-party-only publishing model") and drops out of this wave. |
| 4 | MVP-007, MVP-012, MVP-021 | Checkout needs accounts + product-detail evidence fields; product/release editor needs the file pipeline (MVP-006) **and the existing `ADMIN` authorization pattern — no longer MVP-011** (reworded 2026-09-24, same decision); SEO metadata needs product-detail fields. |
| 5 | MVP-008, MVP-014, MVP-019 | Webhook fulfillment needs checkout. **MVP-014 and MVP-019 moved up from Wave 6** (reworded 2026-09-24, `docs/final-decisions.md`, "First-party-only publishing model") — both now depend on MVP-012 (Wave 4), not the superseded MVP-013, so neither waits for a moderation queue that no longer exists in this form. ~~MVP-013~~ **is Superseded and drops out of this wave**; its quality requirements are redistributed per `docs/final-decisions.md`, section 5. |
| 6 | MVP-009 | Signed downloads need scan pipeline + verified webhook fulfillment. |
