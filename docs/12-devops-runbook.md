...
## Incident priorities
P0 security/payment/data exposure; P1 checkout/download outage; P2 degraded search, notifications or ~~creator~~ first-party publishing (reworded 2026-09-24, `docs/final-decisions.md`, "First-party-only publishing model") workflow; P3 minor content/admin issue. Runbooks cover webhook backlog, scan backlog, failed deployment, database saturation, object-storage failure and ~~compromised creator account~~ **compromised administrator account** (reworded, same decision — arguably a more severe incident class now, since the `ADMIN` role is the sole authoring/publishing authority; worth its own explicit runbook entry, not assumed identical to the old creator-account risk).
